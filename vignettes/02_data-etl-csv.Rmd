...
This vignette documents the steps to ingest model output files (in CSV format) for use in the dashboard. The next [vignette](./03_data-etl-netcdf.html) describes the approach used to ingest and post-process some of the gridded NetCDF output files.
...
```{r, fig.asp=.9}
ken_2[period=="month" & class %in% ken_2_m[, unique(class)][1]] %>% 
  ggplot() +
  geom_line(aes(as.Date(paste(year, month, 1), "%Y %m %d"), value, color=subclass)) +
  facet_grid(paste(class, subclass, variable)~., scales="free_y") +
  th() +
...
[^fn3]: This is not an ideal approach, as we'd rather avoid replicating any hydrological model logic in the dashboard code. TBD for a future version.

## Sheet #1

Field mappings are constructed in shown table below.
...
Formulas for calculated fields are compiled below.
...
We proceed similarly for Sheet #2 and Sheet #3 variable mappings and calculated fields.
...
Finally we add human-readable variable labels.
