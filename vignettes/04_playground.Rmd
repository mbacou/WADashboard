...
<span class="bg-warning">Note: `available_water` as `exploitable_water - non_uti - reserved_outflow` is negative. shall we use `consumed_water` or another definition?</span>
...
# Yearly
dt <- data[id %in% c("exploitable_water", "reserved_outflow") & period=="year"]
dt <- dcast(dt, ...~id)[, value := 1E9 * (exploitable_water) / tot_pop]
...
```{r, layout="l-screen"}
# Monthly profile
dt <- data[id %in% c("exploitable_water") & period=="month"]
dt <- dcast(dt, ...~id)[, value := 1E9 * exploitable_water / tot_pop]
...
plot_wheel(iso3="ken", subset="date_end=='2003-12-31'",
  title="System Water Uses",
  subtitle="Kenya 2003")

plot_wheel(iso3="ken", subset="date_end=='2017-12-31'",
  title="System Water Uses",
  subtitle="Kenya 2017")
```

`((Managed water use[^s2] – non ag classes[^s2]) / water consumed[^s1])`

With `Non ag classes = (settlements + residential + industry + indoor domestic + indoor industry + power + energy + all_others)`

<span class="bg-warning">Note: verify definition of `all others` classes.</span>

```{r, eval=FALSE}
# Yearly time-series
dt <- data[id %in% c("malu_et", "consumed_water") | 
    (id %like% "settlement") | (id %like% "residential") | (id %like% "industry") |
    (id %like% "domestic") | (id %like% "power") | (id %like% "others")
    & period=="year"]

dt <- dcast(dt, ...~id)[, value := 100 * malu_et,
    - molu_settlements_et - malu_residential_et - 
    - malu_industry_et - malu_iiindustry_et -
    - malu_idomestic_et - malu_powerandenergy_et -
    - malu_others1_et - malu_others2_et, -molu_others_et), na.rm=T) / consumed_water]
...
plot_ts(dt, name="Precipitation", unit="km³",
  title="Precipitation", subtitle="Kenya 2003-2017 (MCM / month)")
```
