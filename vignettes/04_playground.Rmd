...
plot_profile <- function(data, unit=NA, yrange=NULL, ...) {
  ...
  highchart2(type="stock") %>%
    ...
    hc_xAxis(type="datetime", dateTimeLabelFormats=list(month="%e %b"),
      plotBands=xBands) %>%
    hc_yAxis(min=yrange[1], max=yrange[2]) %>%    
    hc_rangeSelector(enabled=FALSE, inputEnabled=FALSE) %>%
    hc_navigator(enabled=FALSE) %>%
    hc_scrollbar(enabled=FALSE) %>%    
    hc_themed(...)
...
# => seasonal series are not ingested
...
<span class="bg-warning">Note sure whether to use 0 (zero) as a threshold or a low value (e.g. < 5%).</span>
...
plot_ts(dt, name="Insuffient Flow", unit="%",
  ...
  subtitle=sprintf("Kenya 2003-2017 (%% of months in period below %s)", percent(threshold))
)
```
