...
plot_profile <- function(data, unit=NA, ...) {
  ...
  ymax = copy(data)[, max(year(date_start))]
...
Collection of visual concepts for WA Dashboard.
...
ind[, scoreBox(
  value = sprintf('<span class="">%s %% / year</span>', value),
  subtitle = sprintf('<span class="">+/- %s %% / year</span>', sd),
  footer = "Percent Closure")]
```
...
```{r, results='asis', eval=F}
ind[, scoreBox(
  value = sprintf('<span class="">%s %%</span>', value),
  subtitle = sprintf('<span class="">+/- %%</span>', sd),
  footer = "Agricultural Water Use")]
```
