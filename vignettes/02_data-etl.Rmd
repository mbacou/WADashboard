...
We start below by inspecting all basin-level variables contained in the modeled CSV files.
...
As mentioned above we have 8 yearly water balances for Mali (Sheet #1 variables), 15 yearly balances for Kenya (across `r data[iso3=="ken" & is.na(month), uniqueN(sheet)]` sheets), as well as monthly and seasonal breakdowns. 
...
We (arbitrarily) gather all the seasonal time-series[^fn1]:
...
```{r}
fread(system.file("./csv/sheet_1_schema.csv", package="WADashboard")) %>% 
  paged_table()
```
